/**
 * "Menaxho kategoritë" for the blog — the same dialog the training editor got, against
 * the post taxonomy.
 *
 * It lives beside the Kategoria select in the post editor for the same reason: Settings
 * is `ADMIN_ONLY` in `nav-items.ts`, but the API lets an EDITOR create and rename a
 * category, and the moment you discover a category is missing is the moment you are
 * filling in that field.
 *
 * DELETION IS DISCOVERABLE BUT NOT ACCIDENTAL. Rendered only for an ADMIN (the server
 * returns 403 for anyone else), at the end of a row rather than in the flow of adding
 * one, behind the same `ConfirmDialog` every other destructive action uses. A category
 * still in use is refused by the API with a sentence naming the count; that message is
 * rendered verbatim rather than reworded, so the wording lives in exactly one place.
 */
...
type Props = {
  ...
  readonly onCreated?: (category: PostCategory) => void;
};
...
export function CategoryManagerDialog({ open, onOpenChange, onCreated }: Props): JSX.Element {
  ...
  const invalidate = async (): Promise<void> => {
    await queryClient.invalidateQueries({ queryKey: queryKeys.postCategories.all });
    void queryClient.invalidateQueries({ queryKey: queryKeys.posts.all });
  };

  const createMutation = useMutation({
    mutationFn: (name: string) => createPostCategory({ name }),
    /*
  ...
  const renameMutation = useMutation({
    mutationFn: ({ id, name }: { id: string; name: string }) => updatePostCategory(id, { name }),
    onSuccess: (category) => {
      void invalidate();
      setEditingId(null);
  ...
  const deleteMutation = useMutation({
    mutationFn: (category: PostCategory) => deletePostCategory(category.id),
    onSuccess: () => {
      void invalidate();
      ...
      <Dialog open={open} onOpenChange={onOpenChange}>
        <DialogContent className="sm:max-w-lg">
          <DialogHeader>
            ...
            <DialogDescription>
              Kategoritë shfaqen si filtra në faqen publike të lajmeve. Emri që shkruani
          ...
          <form
            ...
            onSubmit={(event) => {
              ...
              /*
                STOPS PROPAGATION, and this is load-bearing. Radix portals this dialog to
                document.body, so it is not a DOM descendant of the post editor's <form> —
                but React dispatches events through the REACT tree, not the DOM one, and
                this dialog IS a React child of that form. Without this, pressing "Shto"
                submitted the article as well as creating the category.
              */
              ...
              event.stopPropagation();
                  ...
                  >
                    ...
                    {/*
                      ...
                      predictable BEFORE it is pressed: a category showing "2 artikuj"
                      is visibly one the API will refuse to remove. Drafts count too —
                      an unpublished post holds the foreign key exactly as a published
                      one does, so the number matches what the API will enforce.
                    */}
                    <Badge variant={category.postCount > 0 ? 'default' : 'muted'}>
                      {category.postCount === 1
                        ? '1 artikull'
                        : `${category.postCount} artikuj`}
                    </Badge>
                    <Button
                      type="button"
                      size="icon"
          ...
          <>
            ...
            {pendingDelete && pendingDelete.postCount > 0 ? (
              <p className="mt-2">
                ...
                {pendingDelete.postCount === 1
                  ? '1 artikull'
                  : `${pendingDelete.postCount} artikuj`}
